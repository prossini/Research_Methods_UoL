...
For some basic information and guidance about R and RStudio, check out this tutorial:
...
First things first: whenever you open R to work on something, you need to load the necessary packages. For this workshop, we will use the packages below. 
...
# now the correlation
cor.test(df$F_EDUCCAT2, df$AVOIDPT1_W74R, use='complete.obs') # the use = complete.obs is to remove missing values from the data.

  
```

We find a negative correlation (-0.024) that is significant (p = 0.04) between educational levels and frequency of online posting, meaning that there is an inverse relationship between variables (one variable increases and the other decreases). 
In this example, this can be interpreted as: higher education is correlated with lower frequency of social media political expression (or the  other way around). 
