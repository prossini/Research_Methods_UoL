...
In R, you can select a column (variable) in a dataframe using the operator $ after the dataframe name. 
In this working example, we named our dataframe "df", so we can call any column by using df$variable. Note that "df" is an arbitrary name: you can select any name when you assign the external file to an R object using the operator <- and the command 'read_sav'. 
